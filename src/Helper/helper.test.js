...
describe("_validateArgs test", () => {
  test("should throw an error if the arguments is not as we expected", () => {
    ...
    expect(() =>
      _validateArgs(undefined, undefined, undefined, true)
    ).toThrowError(UtilError);
    expect(() =>
      _validateArgs("Romana", "the value must be string or number", types, true)
    ).not.toThrowError(UtilError);
    expect(() =>
      _validateArgs(1, "the value must be string or number", types, true)
    ).not.toThrowError(UtilError);
    expect(() =>
      _validateArgs(true, "the value must be string or number", types, false)
    ).not.toThrowError(UtilError);
  });
});
